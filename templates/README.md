...
> **一句话**：`templates/` 只提供**骨架**——有哪些文件、放在哪、叫什么、块的顺序。
> 「内容怎么写」归 **skills**，「规则是什么」归 **spec**。

## 模板铁律（只放骨架）

1. **不得出现具体文案示例**：没有示例世界观、示例作品、示例角色、示例句子、示例标签值。
   一律用 `{占位符}`（`{中文标题}`、`{世界观 A}`、`{类型}`、`{tag}`…）。
2. **不得出现指令性散文**：`（正文段落）`、`（这里写…）` 这类提示属于 skill，不属于模板；
   模板里只留块标题与 `{…}` 占位符。
3. **不得写「怎么写」**：写法在 `.dsh/skills/story-craft`，格式规则在 `docs/spec/`。
4. 允许保留：块标题、固定标签（如分级警告行）、目录与文件结构、`{…}` 占位符、版本与许可行。

> 这三条由 `node scripts/qa/check-format.js .` 的**参照骨架自检**机器把关：
> 模板出现指令性散文、或出现具体世界观/作品实例，直接 FAIL。
...
└── repo-root-README.md           # 仓库根 README 骨架（唯一非世界观模板，仅小节框架 + 占位符）
```
