...
| 兽种 | 已建档数 | 委托图 | 说明 |
|------|---------|-------|------|
| [熊科 / Bear](bear/) | 89 | 🖼 2（1 同框） | 熊兽人（棕熊、白熊、黑熊、灰熊等）——兽盾主力兵种，分布最广 |
| [牛科 / Bull](bull/) | 67 | 🖼 2 | 牛兽人（黄牛、水牛等）——储精量最大，生命力顽强 |
| [虎科 / Tiger](tiger/) | 72 | 🖼 2 | 虎兽人（黄虎、白虎、蓝虎、黑虎等）——精锐战士，战斗技巧丰富 |
| [犬科 / Dog](dog/) | 20 | 🖼 2 | 狗兽人（德牧、哈士奇等）——数量庞大，多任底层巡逻 |
| [狼科 / Wolf](wolf/) | 15 | 🖼 1 | 狼兽人——敏捷见长，多担任巡逻 / 狙击 |
| [野猪 / Boar](boar/) | 20 | 🖼 1 | 野猪兽人——体格强壮，獠牙突出 |
| [狮科 / Lion](lion/) | 9 | 🖼 1 | 狮兽人——力大无穷，身体素质最为出色 |
| [豹科 / Leopard](leopard/) | 9 | 🖼 1 | 豹兽人——速度快、敏捷高，多任侦查 |
| [犀牛 / Rhino](rhino/) | 14 | 🖼 1 | 犀牛兽人——皮糙肉厚，正面防御角色 |
| [混合 / Mixed](mixed/) | 27 | — | 多兽种混合场景 / 集体档案 |
| [特殊 / Special](special/) | 16 | — | 部长级、组长级、高层特殊角色 |
| **合计** | **358** | 🖼 12 | 每个兽种/等级的 README 表格行数已与档案文件数逐一核对一致 |

> 🖼 = 该兽种已配委托稿张数（合计 **12 张 / 9 个兽种**）· 同框 = 该图中作为同框角色入镜 · — = 暂无委托稿。
> 逐张浏览见下方 [委托稿配图索引](#委托稿配图索引)。
> **「已建档数」的口径（2026-09-17 起改用单一口径）**：该兽种目录下的 `.md` 档案**文件总数**（不含各级 `README.md`），由脚本与文件系统逐目录双向核对。
> 其中标题以 **⭐** 开头的为**集体档案**（一份覆盖多名无个体描写差异的群演），已在各兽种 README 的「总览」表中逐行标出——**本表不再单独拆分 `N+M⭐`**，因为「⭐」在档案标题与索引行之间标记并不总是一致，拆分会持续产生对不上的数字。

> **`N+M⭐`** 的计数口径：`N` = 独立个体档案数，`M` = 标题以 **⭐** 开头的**集体档案**数（一份集体档案覆盖多名无个体描写差异的群演），`N+M` = 该兽种目录下的 `.md` 文件总数。此列现由脚本从文件系统与各级 README 表格双向核对生成。
...
| **墨犬篇** | Ch.1 ~ Ch.2 | ✅ **全部完成** | 失忆武者黑狗的洞穴醒来与山间遭遇；两章均为「主角 vs 单兵」二人场景（含独立建档的洞穴身份未明尸体） |
| **老雷突袭情报局篇** | ch-1 ~ ch-4 | ✅ **全部完成** | 老雷清剿兽盾情报部门 + 情报部长颜某清算值守兵 + 围剿反杀 + 暗室三座浪青平登场；颜部长率众合围、以绿制服犀牛杂兵为饵引爆闪光弹重创浪青平，以及牛棚/杂物间线的搜捕反杀（浪青平被捅伤后藏身牛棚杂物间，老雷从背后握嘴撸杀黑制服熊兽武斗兵） |
...
>
> *总索引最后更新：2026-09-17 · 已建档数：**358 个档案文件**（每个兽种/等级的 README 表格行数已与文件数双向核对一致）*
>
